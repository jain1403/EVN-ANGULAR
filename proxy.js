...
var setUaaConfig = function(options) {
	...
	base64ClientCredential = options.base64ClientCredential;
};
...
function buildDecorator(zoneId, flag) {
	var decorator = function(req) {
		if (corporateProxyAgent && process.env.node_env==='development') {
			console.log('Corporate Environment');
			...
			if(checkNullAndUndefined(req.session.passport, 'user')){
				if(checkNullAndUndefined(req.session.passport.user, 'ticket')){
						if(checkNullAndUndefined(req.session.passport.user.ticket, 'access_token')){
							req.headers['Authorization']='bearer %s'.replace('%s', req.session.passport.user.ticket.access_token);
					}	
				}
			}
		}
...
var setBackendProxyMiddleware = function(configurationObject, pathPrefix, endpoint){
	var set_https = false;
	if(endpoint.indexOf('https')!==-1){
		set_https = true;
	}
	if(configurationObject.uaaIsConfigured){
		//If UAA Is configured then add the token in headers
		configurationObject.app.use(pathPrefix, configurationObject.passport.authenticate('main', {noredirect: true}), function(req, res, next){
			req = addUserTokenToRequest(req);
			next();
		},expressProxy(endpoint, {
			https: set_https,
			forwardPath: function(req, res) {
				return require('url').parse(req.url).path;
			},
			intercept: cleanResponseHeaders,
			//decorateRequest: buildDecorator(null, false)
		}));
	}else{
		configurationObject.app.use(pathPrefix, expressProxy(endpoint, {
			https: set_https,
			forwardPath: function(req, res) {
				return require('url').parse(req.url).path;
			},
...
module.exports = {
	router: router,
	setBackendProxyMiddleware: setBackendProxyMiddleware,
